...
def search_l3(keyword, as_json=False):
    """Search L3 Knowledge Layer via grep."""
    config = load_config()
    l3_paths = config.get("l3_paths", [])

    if not l3_paths:
        msg = "L3: skipped (config not found or l3_paths empty — edit ~/.ai-memory/config.json)"
        if as_json:
    ...
    if as_json:
        return {"layer": "L3", "status": "ok", "results": all_results}

    if not all_results:
        print(f"L3: no matches for '{keyword}'")
        return None

    print(f"L3 (Knowledge Layer) — {len(all_results)} match(es):")
    ...
    if args.as_json:
        ...
        for layer in layers_to_search:
            if layer == "L1":
                ...
                output["layers"].append(search_l3(args.keyword, as_json=True))
